        ...
        <div v-show="!this.boardShow">
            ...
            <el-col :span="18">
                ...
                <div class="tools">
                    ...
                    <el-button v-show="!this.boardShow" @click="openBoard">
                        使用白板
                    </el-button>
                    <el-button v-show="this.boardShow" @click="closeBoard">
                        关闭白板
                    </el-button>

                    <el-button @click="dianming">随机点名</el-button>

                    <el-button @click="chat">聊天</el-button>

                    <el-badge :value="this.handsNum" class="item">
                        <el-button size="small" @click="seeHands">举手</el-button>
                    </el-badge>

                    <el-button v-show="!isRecording" @click="record">录制</el-button>
                    <el-button v-show="isRecording" @click="stopRecord">停止录制</el-button>

                    <button @click="stop" class="stopbtn">结束直播</button>
                </div>
            </el-col>
            <el-col :span="6">
                <div id="rside">
                            ...
                            <div v-show="!agree">
                                <div v-for="student in students" :key="student.msg">
                                    <div class="handinfo">
                                        <h4>{{ student.msg }}</h4>
                                        <p>同学举手</p>
                                        <el-button @click="agreehand(student)">同意</el-button>
                                        <el-button @click="closeHand(student)">取消</el-button>
                                    </div>
                                </div>
                            </div>
                            <div v-show="agree">
                                正在与{{ lianmaistudent }}连麦
                                <el-button @click="closeHand(lianmaistudent)">关闭</el-button>
                            </div>
                        </div>
                        <!-- 不举手就默认是聊天 -->
                        <div v-show="!handShow">
        ...
        <div class="drawBoard" v-show="this.boardShow">
            ...
            <div>
                <div id="board" class="canvas-layers">
                    <canvas id="drawBoard" width="1100" height="650" style="position:absolute;"></canvas>
                    <canvas id="showBoard" width="1100" height="650" style="position:absolute;"></canvas>
                </div>
            </div>
            <!-- 工具栏 -->
...
export default {
    ...
    methods: {
        ...
        connect() {
            ...
            this.socket = new WebSocket(
                "ws://127.0.0.1:8000/" + this.uid + "/room" + this.cid + "room" + "/"
            );
            ...
            this.socket.onmessage = (event) => {
                ...
                if (messageData.kind == "chatmessage") {
        ...
        startPlayCamera() {
            const cameraUrl =
                "https://zlm.com/index/api/webrtc?app=live&stream=" +
                this.cid +
                "teacherCamera&type=push";
        ...
        agreehand(student) {
            if (!this.agree) {
                this.agree = true;
                this.drawws.send(
                    "permit," + student["msg"] + "," + student["image"] + ","
                );
        ...
        closeHand(student) {
            ...
            this.agree = false;
            this.handShow = false;
            this.students = this.students.filter(s => s.msg !== student);
            const divB = this.$refs.message.offsetHeight;
            this.$refs.message.style.height = "420px";
            this.have2 = false;
        ...
        clearDraw() {
            ...
            this.showctx.clearRect(
                0,
                0,
                this.showcanvas.width,
                this.showcanvas.height
            );
        },
        mydraw(msg, ctx1) {
        ...
        chat() {
            this.handShow = false;
            const divB = this.$refs.message.offsetHeight;
            this.$refs.message.style.height = "220px";
        },
        record() {
            axios
                .get(
                    "https://zlm.com/index/api/startRecord?secret=x5mji1hHaDRVAsMCYUIrChqhmcgbozR1&type=1&vhost=192.168.44.129&app=live&stream=" +
                    this.cid +
                    "desk",
                    {
                        headers: {
        ...
        stopRecord() {
            axios
                .get(
                    "https://zlm.com/index/api/stopRecord?secret=x5mji1hHaDRVAsMCYUIrChqhmcgbozR1&type=1&vhost=192.168.44.129&app=live&stream=" +
                    this.cid +
                    "desk",
                    {
                        headers: {
        ...
        getFileList() {
            ...
            axios
                .get(
                    "https://zlm.com/index/api/getMp4RecordFile?secret=x5mji1hHaDRVAsMCYUIrChqhmcgbozR1&vhost=192.168.44.129&app=live&stream=" +
                    this.cid +
                    "desk&customized_path=&period=" +
                    today,
                    {
                        headers: {
                            "Content-Type": "multipart/form-data",
    ...
    mounted: function () {
        ...
        this.drawws = new WebSocket(
            "ws://127.0.0.1:8000/" + this.uid + "/canvas" + this.cid + "/"
        );
...
class canvasDraw {
    ...
    drawLine() {
        ...
        this.canvas.onmouseup = () => {
            ...
            this.ctx.stroke();
            this.ws.send(
                `draw,line,${this.path.beginX},${this.path.beginY},${this.path.endX},${this.path.endY},${this.ctx.strokeStyle},${this.ctx.fillStyle},${this.ctx.lineWidth},${this.ctx.lineCap},${this.ctx.lineJoin}`
            );
        };
    }
    drawRect() {
        ...
        this.canvas.onmouseup = () => {
            ...
            this.ctx.stroke();
            this.ws.send(
                `draw,rect,${this.path.beginX},${this.path.beginY},${this.path.endX - this.path.beginX
                },${this.path.endY - this.path.beginY},${this.ctx.strokeStyle},${this.ctx.fillStyle
                },${this.ctx.lineWidth},${this.ctx.lineCap},${this.ctx.lineJoin}`
            );
        };
    }
    drawRound() {
        ...
        this.canvas.onmouseup = () => {
            ...
            this.ctx.stroke();
            this.ws.send(
                `draw,round,${this.path.beginX + width / 2},${this.path.beginY + height / 2
                },${Math.sqrt(width * width + height * height) / 2},0,${this.ctx.strokeStyle
                },${this.ctx.fillStyle},${this.ctx.lineWidth},${this.ctx.lineCap},${this.ctx.lineJoin
                }`
            );
        };
    }
    ...
    drawing(e) {
        ...
        this.ws.send(
            `draw,${this.path.beginX},${this.path.beginY},${this.path.endX},${this.path.endY},0,${this.ctx.strokeStyle},${this.ctx.fillStyle},${this.ctx.lineWidth},${this.ctx.lineCap},${this.ctx.lineJoin}`
        );
    }
    clearCanvas() {
...
.canvasSetting input[type="range"]::-webkit-slider-thumb {
    -webkit-appearance: none;
    appearance: none;
    width: 15px;
    height: 15px;
...
.canvasSetting input[type="range"]::-moz-range-thumb {
    width: 15px;
    height: 15px;
    border-radius: 50%;
    background: #4CAF50;
...
#cameraVideo::-webkit-media-controls {
    display: none !important;
}

#cameraVideo::-webkit-media-controls-enclosure {
    display: none !important;
}
